...
`PerconaXtraDBVersion` is a Kubernetes `Custom Resource Definitions` (CRD). It provides a declarative configuration to specify the docker images to be used for PerconaXtraDB deployed with KubeDB in Kubernetes native way.

When you install KubeDB, `PerconaXtraDBVersion` object will be created automatically for every supported PerconaXtraDB versions. You have to specify the name of `PerconaXtraDBVersion` object in `.spec.version` field of [PerconaXtraDB](/docs/concepts/databases/percona-xtradb.md) object. Then, KubeDB will use the docker images specified in the correspondinh `PerconaXtraDBVersion` object to create your expected database.
...
`.metadata.name` is a required field that specifies the name of the `PerconaXtraDBVersion` object. You have to specify this name in `.spec.version` field of [PerconaXtraDBVersion](/docs/concepts/databases/percona-xtradb.md) object.
...
We modify original PerconaXtraDB docker image to support additional features. An image with higher modification tag will have more feature than the images with lower modification tag. Hence, it is recommended to use PerconaXtraDBVersion object with highest modification tag to take advantage of the latest features.
...
- Learn about PerconaXtraDB CRD [here](/docs/concepts/databases/percona-xtradb.md).
- Deploy your first PerconaXtraDB database with KubeDB by following the guide [here](/docs/guides/percona-xtradb/percona-xtradb-cluster.md).
